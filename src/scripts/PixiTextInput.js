...
PixiTextInput.prototype.onBackgroundMouseDown = function(e) {
  ...
  const x = this.toLocal(e.data.global).x;
...
PixiTextInput.prototype.handlePaste = function(e) {
  const txtToInsert = e.clipboardData.getData('text/plain');
  if (this._selection) {
    this.deleteSelectedText();
    this.moveCarretRight();
  }
...
PixiTextInput.prototype.updateCaretPosition = function() {
  ...
  const sub = this._value.substring(0, this._caretIndex).substring(this.scrollIndex);
...
PixiTextInput.prototype.drawElements = function() {
  ...
  if (this._selection && this._caretIndex != this._secondCaretIndex) {
    ...
    const sub = this._text.substring(selectionStart, selectionEnd);
    let selectedWidth = this.textField.context.measureText(sub).width;
...
PixiTextInput.prototype.getCaretIndexByCoord = function(x) {
  let smallest = 10000;
  let cand = 0;
  const visible = this._text.substring(this.scrollIndex);
